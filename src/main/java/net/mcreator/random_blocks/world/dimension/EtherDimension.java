...
package net.mcreator.random_blocks.world.dimension;
...
import net.mcreator.random_blocks.item.EtherItem;
import net.mcreator.random_blocks.block.PoisonedwaterBlock;
...
public class EtherDimension extends RandomBlocksModElements.ModElement {
	@ObjectHolder("random_blocks:ether")
	public static final ModDimension dimension = null;
	@ObjectHolder("random_blocks:ether_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("ether"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("random_blocks:ether")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("random_blocks:ether"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("random_blocks:ether"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("random_blocks:dust")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new EtherItem().setRegistryName("ether"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = EtherDimension.CustomPortalBlock.createPatternHelper(entity.world, new BlockPos(pos));
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == Blocks.END_STONE_BRICKS.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == Blocks.END_STONE_BRICKS.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.END_STONE_BRICKS.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "ether_portal",
	...
	public static class TeleporterDimensionMod extends Teleporter {
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? Blocks.END_STONE_BRICKS.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.END_STONE_BRICKS.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = EtherDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
		...
		public ChunkProviderModded(World world, BiomeProvider provider) {
			super(world, provider, new NetherGenSettings() {
				...
				public BlockState getDefaultFluid() {
					return PoisonedwaterBlock.block.getDefaultState();
				}
			});
