...
import net.mcreator.random_blocks.block.DeadoakBlock;
...
public class DeadplainBiome extends RandomBlocksModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.PLAINS).waterColor(-6710887).waterFogColor(-6710887).parent("plains")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.DIRT.getDefaultState(), Blocks.DIRT.getDefaultState())));
			setRegistryName("dead_plains");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(DeadoakBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(DeadleavesBlock.block.getDefaultState()))).baseHeight(4)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DeadoakBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DeadoakBlock.block.getDefaultState().getBlock()
					|| blockType == DeadleavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
